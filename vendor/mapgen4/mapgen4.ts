...
function main({mesh, t_peaks}: { mesh: Mesh; t_peaks: number[]; }) {
    ...
    const mapId = new URLSearchParams(window.location.search).get('mapId') ?? 'default';
    const storageKey = `world-archive-mapgen4:${mapId}`;
    try {
        const saved = JSON.parse(localStorage.getItem(storageKey) ?? 'null');
        if (saved?.param) {
            for (const phase of ['elevation', 'biomes', 'rivers', 'render']) {
                for (const [name, value] of Object.entries(saved.param[phase] ?? {})) {
                    if (typeof value !== 'number' || !(name in param[phase])) continue;
                    param[phase][name] = value;
                    const input = document.querySelector(`#slider-${name} input`) as HTMLInputElement | null;
                    if (input) input.value = String(value);
                }
            }
        }
        if (Array.isArray(saved?.constraints)) Painting.restoreConstraints(saved.constraints);
    } catch (error) {
        console.warn('Mapgen4 state restore failed', error);
    }
    ...
    function saveState() {
        try {
            localStorage.setItem(storageKey, JSON.stringify({
                param,
                constraints: Array.from(Painting.constraints),
                savedAt: new Date().toISOString(),
            }));
        } catch (error) {
            console.warn('Mapgen4 state save failed', error);
        }
    }
