...
export function ProjectExplorer({ project, onOpen, onAddMap, onChange }: Props) {
  ...
  const [categoryNotice, setCategoryNotice] = useTransientMessage();
  ...
  const renderCategory = (category: WikiCategoryDefinition, depth = 0): ReactNode => {
    ...
    const button = <button type="button" className={className} onClick={() => hasChildren ? toggle(category.id) : openLeafCategory(category)}>
      <span className="category-node-label"><b aria-hidden="true">{hasChildren ? (isExpanded ? "▾" : "▸") : depth < 2 ? "▤" : ""}</b><span className="category-node-name" data-user-authored="true" title={category.name}>{category.name}</span></span>
      <small>{count}</small>
    </button>;
    return (
  ...
  return (
    <aside className="project-explorer">
      <div className="explorer-title"><span>프로젝트</span><strong data-user-authored="true">{project.title}</strong></div>
      <nav className="project-section-tabs" aria-label="프로젝트 영역">
        <button type="button" aria-label={t("projectSection.map")} className={projectSection === "map" ? "active" : ""} onClick={() => setProjectSection("map")}><span className="project-section-label">{t("projectSection.map")}</span></button>
        <button type="button" aria-label={t("projectSection.world")} className={projectSection === "world" ? "active" : ""} onClick={() => setProjectSection("world")}><span className="project-section-label">{t("projectSection.world")}</span></button>
        <button type="button" aria-label={t("projectSection.environment")} className={projectSection === "environment" ? "active" : ""} onClick={() => setProjectSection("environment")}><span className={`project-section-label${language === "en" ? " environment-compact" : ""}`}>{t("projectSection.environment")}</span></button>
      </nav>
      ...
      {projectSection === "map" && <section className="tree-section explorer-section-pane">
        ...
        {project.maps.map((map) => <button type="button" className="tree-item" key={map.id} onClick={() => onOpen("map", map.id)}><span>▧</span><span data-user-authored="true">{map.title}</span><small>{generatedAtYear(map) ? "현재 연도 생성됨" : "현재 연도 빈 지도"}</small></button>)}
      ...
      {projectSection === "world" && <section className="tree-section world-tree-section explorer-section-pane">
        ...
        {editingCategories && <div className="category-inline-editor">
          ...
          {showNewCategory && <div className="new-category-panel">
            ...
            <label>상위 카테고리<select value={newCategoryParentId} onChange={(event) => setNewCategoryParentId(event.target.value)}><option value="">최상위</option>{project.wikiCategories.map((category) => <option data-user-authored="true" key={category.id} value={category.id}>{category.name}</option>)}</select></label>
      ...
      {projectSection === "environment" && <section className="tree-section environment-tree-section explorer-section-pane">
        ...
        {project.maps.map((map) => <button type="button" className="tree-item environment-tree-item" key={map.id} onClick={() => onOpen("simulation", map.id)}><span>◉</span><span data-user-authored="true">{map.title}</span><small>{map.environmentPins.length}개 핀</small></button>)}
      </section>}
    </aside>
  );
}
