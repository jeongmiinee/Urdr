...
function AssessmentTable({ title, items, freeMode }: { title: string; items: AgricultureAssessment[]; freeMode: boolean }) {
  return (
    <section className="simulation-card assessment-section">
      <div className="simulation-card-heading"><div><p className="eyebrow">{title === "작물" ? "CROPS" : "LIVESTOCK"}</p><h3>{title} 환경 적합도</h3></div>{freeMode && <span className="reference-badge">참고값</span>}</div>
...
function MonthlyTable({ analysis, timelineLabel }: { analysis: EnvironmentLocationAnalysis; timelineLabel: string }) {
  return <section className="simulation-card monthly-climate-card">
    <div className="simulation-card-heading"><div><p className="eyebrow">MONTHLY CYCLE</p><h3>월별 환경 변화</h3></div><span className="reference-badge">{timelineLabel} 기준</span></div>
...
function PinComparison({ map, month, selectedPoint, onSelect, onRemove }: {
  map: MapData;
  month: number;
  selectedPoint: Point;
  onSelect: (point: Point) => void;
  ...
  return <section className="simulation-card pin-comparison-card">
    <div className="simulation-card-heading"><div><p className="eyebrow">PIN COMPARISON</p><h3>고정 지점 비교</h3></div><span className="reference-badge">{MONTH_LABELS[month - 1]}</span></div>
  ...
  return (
    <section className="simulation-window">
      <header className="simulation-header">
        <div><p className="eyebrow">POINT ENVIRONMENT & PRODUCTION</p><h2>선택 지점 환경 분석</h2><p>{freeMode ? "자유 모드: 선택 지점의 환경값은 참고 정보이며 설정을 제한하지 않습니다." : "현실 모드: 선택 지점의 월별 환경값이 작물·목축 생산성의 기본 제약으로 적용됩니다."}</p></div>
      ...
      {!generated || !selectedPoint || !analysis ? <div className="empty-document"><strong>환경 데이터가 없습니다.</strong><p>먼저 지도 생성기에서 지도를 확정하세요.</p></div> : <>
        <section className="environment-analysis-layout">
          <div className="environment-map-card simulation-card">
            <div className="simulation-card-heading"><div><p className="eyebrow">SELECT A POINT</p><h3>지도에서 분석 지점 선택</h3></div><span className="reference-badge">클릭하여 이동</span></div>
          ...
          <aside className="environment-context-card simulation-card">
            <p className="eyebrow">ANALYSIS CONTEXT</p><h3>{pinName.trim() || "현재 선택 지점"}</h3>
            <dl className="environment-location-meta"><div><dt>좌표</dt><dd>X {analysis.position.x.toFixed(2)} · Y {analysis.position.y.toFixed(2)}</dd></div><div><dt>격자</dt><dd>{analysis.gridX}, {analysis.gridY}</dd></div><div><dt>위도</dt><dd>{analysis.latitudeDeg.toFixed(2)}°</dd></div><div><dt>고도</dt><dd>{analysis.elevationM.toLocaleString()}m</dd></div><div><dt>지형</dt><dd>{analysis.isLand ? (TERRAIN_LABELS[analysis.terrain] ?? analysis.terrain) : "해양"}</dd></div><div><dt>타임라인</dt><dd>{formatTimelineMoment(project, map.timeline)}</dd></div></dl>
        ...
        <section className="simulation-card comparison-delta-card"><div className="simulation-card-heading"><div><p className="eyebrow">CHANGE</p><h3>{analysis.comparisonLabel}</h3></div></div><div className="comparison-delta-grid"><span>기온 <strong>{signed(analysis.comparison.temperatureC, "℃")}</strong></span><span>강수 <strong>{signed(analysis.comparison.precipitationMm, "mm")}</strong></span><span>습도 <strong>{signed(analysis.comparison.humidityPercent, "%p")}</strong></span><span>토양수분 <strong>{signed(analysis.comparison.soilMoisturePercent, "%p")}</strong></span><span>수자원 <strong>{signed(analysis.comparison.waterAccessIndex)}</strong></span></div></section>

        <section className="simulation-card annual-range-card"><div className="simulation-card-heading"><div><p className="eyebrow">ANNUAL RANGE</p><h3>연중 최저·최고 및 누적값</h3></div></div><div className="annual-range-grid">
        ...
        <section className="seasonal-summary-grid">{analysis.seasonal.map((season) => <article className={`simulation-card season-${season.season}`} key={season.season}><p className="eyebrow">{SEASON_LABELS[season.season]}</p><h3>{season.meanTemperatureC.toFixed(1)}℃ · {season.precipitationMm.toFixed(0)}mm</h3><p>{season.months.map((month) => MONTH_LABELS[month - 1]).join(" · ")}</p><dl><div><dt>습도</dt><dd>{season.humidityRange[0].toFixed(0)}–{season.humidityRange[1].toFixed(0)}%</dd></div><div><dt>토양수분</dt><dd>{season.soilMoistureRange[0].toFixed(0)}–{season.soilMoistureRange[1].toFixed(0)}%</dd></div><div><dt>적설</dt><dd>{season.snowRange[0].toFixed(0)}–{season.snowRange[1].toFixed(0)}mm</dd></div></dl></article>)}</section>

        <MonthlyTable analysis={analysis} timelineLabel={formatTimelineMoment(project, { ...map.timeline, precision: "year" })} />
        <PinComparison map={map} month={selectedMonth} selectedPoint={selectedPoint} onSelect={setSelectedPoint} onRemove={removePin} />
        ...
        <section className="simulation-card external-model-card"><p className="eyebrow">MODEL NOTE</p><h3>선택 지점 계산 방식</h3><p>현재 지도의 고도·기온·연 강수·습도·풍장·수문 격자를 기준으로 월별 일사, 강수 분배, 강설·융설, 증발산, 토양수분과 수자원 접근성을 계산합니다. 타임라인이 날짜 단위이면 월간값을 일별로 보간하고, 시각 단위이면 일교차·강수 이벤트·풍속 변화를 반영해 같은 좌표의 날씨를 자동 갱신합니다. 외부 모델 격자를 가져온 경우 해당 값이 같은 계산의 기초 자료로 사용됩니다.</p></section>
      </>}
    </section>
  );
}
