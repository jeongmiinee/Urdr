  ...
  return (
    <section className="history-window history-readonly">
      <aside className="history-list-panel">
        <div className="window-heading compact"><h2>연표</h2><p>각 문서의 생몰·건국·멸망·설립 기록과 사건 문서를 한 흐름으로 열람합니다.</p></div>
        ...
        {unknownEvents.length > 0 && <section className="timeline-unknown-panel"><div><h3>시점 미정 사건</h3></div>{unknownEvents.map((event) => <button type="button" key={event.id} onClick={() => selectUnknownEvent(event)}><span>{event.title}</span><small>{EVENT_CATEGORY_LABELS[event.category]}</small></button>)}</section>}
      </aside>

      <article className="chronicle-reader">
        {selected ? <>
          <header className="chronicle-reader-heading"><div><p className="section-meta">{formatWorldYear(project, map, selected.year)}</p><h2>{selected.title}</h2><span>{selected.sourceLabel} · {selected.change}</span></div>{selectedArticle && <button type="button" className="secondary-button" onClick={() => onOpenWikiArticle(selectedArticle)}>원본 문서 열기</button>}</header>
          ...
          {selectedEvent && <>
            ...
            {selectedEvent.participants.length > 0 && <section className="chronicle-read-card"><h3>대상</h3><div className="chronicle-participant-list">{selectedEvent.participants.map((participant) => <article key={participant.id}><strong>{participant.organizationName || "이름 없는 대상"}</strong><span>{participant.representativeLeaderName || participant.keyFigures || "주요 인물 미정"}</span><p>{participant.result || participant.cause || "기록 없음"}</p></article>)}</div></section>}
