...
export const PROGRAM_VERSION = "2.5" as const;
...
export type GeneratorSettings = {
  ...
  agriculturalCountryCount: number;
  coastalCountryCount: number;
  nomadicCountryCount: number;
  mountainCountryCount: number;
  commercialCountryCount: number;
  settlementVillageCount: number;
  settlementTownCount: number;
  settlementCityCount: number;
  settlementCapitalCount: number;
  settlementGenerationYear: number;
  preserveExistingSettlements: boolean;
  regenerateSettlementRoads: boolean;
  /** @deprecated v1.1+ migrates this value into typed country counts. */
  countryCount: number;
  /** @deprecated v1.1 uses one calibrated internal boundary weight. */
  naturalBorderInfluence: number;
  allowExclaves: boolean;
  /** @deprecated v1.1 uses coastalCountryCount. */
  maritimeCountryRatio: number;
  /** @deprecated v1.1 uses nomadicCountryCount. */
  nomadicCountryRatio: number;
  /** @deprecated v1.1 uses mountainCountryCount. */
  ...
  /** Vertical distance from the configured maximum to minimum elevation. */
  elevationRangeM: number;
  /** Multi-scale noise applied to the scalar elevation field (0..1). */
...
export type GeneratedRiverNodeKind =
  | "source"
  | "confluence"
  | "lake-inlet"
  | "lake-outlet"
...
export type GeneratedRiverNode = {
  id: number;
  kind: GeneratedRiverNodeKind;
  point: Point;
  cellIndex: number;
  elevation: number;
  discharge: number;
...
export type GeneratedRiverEdge = {
  id: number;
  sourceNodeId: number;
  targetNodeId: number;
  centerline: Point[];
  ...
  magnitude: number;
  catchmentId: number;
  /** Lake-passage edges preserve graph continuity but are not drawn over water. */
  render: boolean;
  lakeId?: number;
};

export type GeneratedRiverGraph = {
  version: 1;
  nodes: GeneratedRiverNode[];
  edges: GeneratedRiverEdge[];
...
export type GeneratedMapData = {
  ...
  /** Authoritative maximum-detail drainage graph. */
  riverGraph?: GeneratedRiverGraph;
  /** Display segments derived from riverGraph for backward-compatible consumers. */
...
export type SettlementDistrictKind =
  | "administrative"
  | "market"
  | "residential"
  | "artisan"
  ...
  | "harbor"
  | "agricultural_edge";
export type SettlementVectorPlan = {
  version: 1;
  seed: number;
  widthM: number;
  heightM: number;
...
export type LocationState = {
  ...
  description: string;
  settlementPlan?: SettlementVectorPlan;
};
...
export type CountryCapitalPeriod = {
  id: string;
  locationId?: string;
  customName?: string;
  startYear?: number;
...
export type CountryProfile = {
  ...
  capitalPeriods?: CountryCapitalPeriod[];
  headOfStateArticleId?: string;
  headOfStateCustom?: string;
  religionArticleIds?: string[];
  predecessorArticleIds?: string[];
  successorArticleIds?: string[];
...
export type TerritoryState = {
  ...
  holes?: Point[][];
  /** Additional disconnected land components owned by the same temporal state. */
  parts?: TerritoryPart[];
  /** Surface-vector version against which coastal edges were clipped. */
  geometryVersion?: number;
  /** Water-mask fingerprint used to invalidate coast-dependent territory geometry. */
...
export type EventParticipant = {
  id: string;
  organizationArticleId?: string;
  organizationName: string;
  representativeLeaderArticleId?: string;
  representativeLeaderName: string;
  otherLeaderArticleIds: string[];
...
export type WorldEvent = TimeRange & {
  ...
  description: string;
  locationMode: "coordinate" | "custom";
  location: Point | null;
  locationText: string;
  cause: string;
  result: string;
...
export const DEFAULT_WIKI_CATEGORY_NAMES: Array<{
  ...
  { id: "wiki-category-technology-root", name: "기술", parentId: "wiki-category-civilization-root" },
  { id: "wiki-category-technology-engineering", systemKey: "technology_engineering", name: "공학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology-mathematics", systemKey: "technology_mathematics", name: "수학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology-science", systemKey: "technology_science", name: "과학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology-chemistry", systemKey: "technology_chemistry", name: "화학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology-medicine", systemKey: "technology_medicine", name: "의학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology-physics", systemKey: "technology_physics", name: "물리학", parentId: "wiki-category-technology-root" },
  { id: "wiki-category-technology", systemKey: "technology", name: "기타 기술", parentId: "wiki-category-technology-root" },
  {
  ...
  { id: "wiki-category-culture-root", name: "문화", parentId: "wiki-category-civilization-root" },
  { id: "wiki-category-culture-sphere", systemKey: "culture_sphere", name: "문화권", parentId: "wiki-category-culture-root" },
  { id: "wiki-category-culture", systemKey: "culture", name: "문화", parentId: "wiki-category-culture-root" },
  {
  ...
  { id: "wiki-category-language-root", name: "언어", parentId: "wiki-category-civilization-root" },
  { id: "wiki-category-language-family", systemKey: "language_family", name: "어족", parentId: "wiki-category-language-root" },
  { id: "wiki-category-language-branch", systemKey: "language_branch", name: "어파", parentId: "wiki-category-language-root" },
  { id: "wiki-category-language-group", systemKey: "language_group", name: "어군", parentId: "wiki-category-language-root" },
  { id: "wiki-category-language", systemKey: "language", name: "언어", parentId: "wiki-category-language-root" },
  { id: "wiki-category-dialect", systemKey: "dialect", name: "방언", parentId: "wiki-category-language-root" },
  { id: "wiki-category-writing-system", systemKey: "writing_system", name: "문자", parentId: "wiki-category-language-root" },
  {
  ...
  {
    id: "wiki-category-item-root",
    name: "물건",
    parentId: "wiki-category-assets-root",
  },
  {
    id: "wiki-category-weapon",
    name: "무기",
    templateKey: "item",
    parentId: "wiki-category-item-root",
  },
  {
    id: "wiki-category-armor",
    name: "방어구",
    templateKey: "item",
    parentId: "wiki-category-item-root",
  },
  {
    id: "wiki-category-accessory",
    name: "장신구",
    templateKey: "item",
    parentId: "wiki-category-item-root",
  },
...
export function createDefaultWikiCategories(): WikiCategoryDefinition[] {
  ...
  return DEFAULT_WIKI_CATEGORY_NAMES.map(
    ({ id, systemKey, templateKey, name, parentId }) => ({
      id,
      name,
      parentId,
      systemKey,
      templateKey: templateKey ?? systemKey ?? "other",
      createdDate: now,
    }),
  );
}
export type PersonProfile = {
  birthYear?: number;
  deathYear?: number;
  nationalityArticleIds?: string[];
  affiliationArticleIds?: string[];
  religionArticleIds?: string[];
  languageArticleIds?: string[];
  ideologyArticleIds?: string[];
  /** @deprecated Import compatibility; migrated into nationalityArticleIds. */
  countryArticleId?: string;
  /** @deprecated Import compatibility; migrated into religionArticleIds. */
  religionArticleId?: string;
  primaryAffiliationArticleId?: string;
  loyaltyByAffiliation?: Record<string, number>;
  /** 종교 연결이 잠시 해제되어도 종교별 신앙심을 보존한다. */
  faithByReligion?: Record<string, number>;
  organizationArticleIds: string[];
  factionArticleIds: string[];
  /** Assigned family document. Its member graph is the authoritative lineage source. */
  familyArticleId?: string;
  /** Editable only while no family document is assigned. */
  privateLineage?: PrivateLineageProfile;
  /** @deprecated Import compatibility; migrated into privateLineage. */
...
export type WikiDocumentSection = {
  id: string;
  title: string;
  content: string;
  /** Stable parent link used by the hierarchical contents editor. */
  parentId: string | null;
  /** Derived depth retained for backward-compatible imports and exports. */
  level: 1 | 2 | 3 | 4;
  /** Legacy field. All sections are included when the project TOC is enabled. */
...
export type DiseaseProfile = {
  ...
  symptomArticleIds: string[];
  /** @deprecated Import compatibility; migrated into symptoms. */
  symptomNotes: string;
  relatedDiseaseArticleIds: string[];
  symptoms?: string[];
  aftereffects?: string[];
  debuffs?: DiseaseDebuffRule[];
  /** @deprecated Import compatibility; migrated into aftereffects. */
  disabilities?: string[];
};

export type DiseaseDebuffRule = {
  /** Must match an active RPG definition label in the native client. */
  target: string;
  operation: "add" | "subtract" | "multiply" | "set";
  value: number;
  unit: "point" | "percent" | "multiplier";
  duration: string;
  stacking: "replace" | "stack" | "strongest";
  condition: string;
};

export type PlaceChronologyEntry = {
  id: string;
  /** Stable standard-orbital year; UI converts this through the active calendar. */
  worldYear: number;
  /** Position within the standard orbital year, in the 0..<1 range. */
  dayFraction: number;
  title: string;
  description: string;
};
...
export type PlaceProfile = {
  chronology: PlaceChronologyEntry[];
};

export type IdeologyProfile = {
  founder: string;
  formationPeriod: string;
  pursuedValues: string;
};

export type LanguageVocabularyEntry = {
  id: string;
  term: string;
...
export type LanguageProfile = {
  familyArticleId?: string;
  branchArticleId?: string;
  groupArticleId?: string;
  parentLanguageArticleId?: string;
  writingSystemArticleIds: string[];
  phonology: string;
  phonotactics: string;
  morphology: string;
  syntax: string;
  ...
  dialects: string;
  vocabulary: LanguageVocabularyEntry[];
};

export type NatureProfile = {
  ...
  birthMethod?: string;
  isLivestock?: boolean;
  /** Legacy natural-history field retained for imports only. */
  distribution?: string;
...
export type PrivateLineageProfile = {
  members: FamilyTreeMember[];
};

export type FamilyProfile = {
  formationPeriod?: string;
  dissolutionPeriod?: string;
  displayMode: "all" | "key";
  members: FamilyTreeMember[];
  displayFlag?: boolean;
  displayCoatOfArms?: boolean;
...
export type ItemSubtype = "weapon" | "armor" | "accessory" | "item";
export type ItemLifecycleStatus = "active" | "retired";
export type ItemProfile = {
  ...
  isUnique: boolean;
  /** 고유 물품에서 사용하는 직접 입력 상태. 이전 condition 값도 여기에 보존한다. */
  condition: string;
...
export type FlagShape = "rectangle" | "rectangle_swallowtail" | "square" | "square_swallowtail" | "triangle";
export type CoatShape = "shield" | "circle" | "square" | "triangle" | "diamond";
export type HeraldicPattern =
  | "solid" | "per_pale" | "per_fess" | "per_bend" | "per_bend_sinister"
  | "quarterly" | "per_saltire" | "per_chevron" | "per_pall" | "gyronny"
  | "barry" | "paly" | "bendy" | "checkered" | "lozengy" | "ermine" | "vair"
  | "chief" | "fess" | "pale" | "bend" | "bend_sinister" | "chevron"
  | "cross" | "saltire" | "pall" | "bordure" | "canton" | "orle"
  | "tressure" | "pile" | "flanches"
  /** v1.7 compatibility */
  | "stripes" | "grid";
export type HeraldicSymbol =
  ...
  | "heart"
  | "fortress_wall"
  | "roundel" | "lozenge" | "mascle" | "annulet" | "sun" | "crescent" | "fleur_de_lis"
  | "lion" | "horse" | "stag" | "boar" | "wolf" | "bear" | "eagle" | "raven" | "fish"
  | "serpent" | "dragon" | "griffin" | "unicorn" | "rose" | "oak_leaf" | "acorn" | "tree"
  | "wheat" | "axe" | "hammer" | "bow" | "shield" | "helm" | "key" | "tower" | "bridge"
  | "ship" | "anchor" | "book" | "scales" | "torch" | "gear";
export type HeraldicLayerKind = "pattern" | "symbol" | "image";
export type HeraldicLayer = {
  id: string;
  kind: HeraldicLayerKind;
  name: string;
  visible: boolean;
  locked: boolean;
...
export type HeraldicAsset = {
  ...
  /** 이전 버전의 전체 이미지 업로드 값. 로드 시 무늬 이미지로 이전한다. */
  imageDataUrl?: string;
  patternImageDataUrl?: string;
  symbolImageDataUrl?: string;
  /** 무늬와 문양을 함께 대체하는 전체 합성 이미지. 외곽 모양은 유지한다. */
  ...
  /** 아래에서 위로 그리는 문장 구성 레이어. */
  layers?: HeraldicLayer[];
  createdAt: string;
  updatedAt: string;
};
...
export type WikiProfileSection = {
  id: string;
  title: string;
  content: string;
  parentId: string | null;
...
export type ReligionDoctrineSection = WikiProfileSection;
...
export type ReligionProfile = {
  ...
  adherentPopulation: string;
  religiousInstitutions: string[];
  scriptures: string[];
  majorDenominations: string[];
  doctrineSections: ReligionDoctrineSection[];
};

export type CultureProfile = {
  distributionRegions: string[];
  relatedLanguageArticleIds: string[];
  relatedReligionArticleIds: string[];
  characteristicSections: WikiProfileSection[];
};
...
export type WikiArticle = {
  ...
  summary: string;
  content: string;
  /** 기존 단일 본문과 함께 사용할 수 있는 사용자 정의 목차 섹션. */
  documentSections?: WikiDocumentSection[];
...
export type WorldProject = {
  ...
  rpgSettings: RpgProjectSettings;
...
export function pointInTerritoryState(
  ...
  return parts.some((part) =>
    pointInPolygon(point, part.polygon) &&
    !(part.holes ?? []).some((hole) => pointInPolygon(point, hole)),
  );
}
...
export function defaultGeneratorSettings(): GeneratorSettings {
  return {
    ...
    agriculturalCountryCount: 1,
    coastalCountryCount: 1,
    nomadicCountryCount: 1,
    mountainCountryCount: 1,
    commercialCountryCount: 1,
    settlementVillageCount: 12,
    settlementTownCount: 6,
    settlementCityCount: 4,
    settlementCapitalCount: 5,
    settlementGenerationYear: 0,
    preserveExistingSettlements: false,
    regenerateSettlementRoads: true,
    countryCount: 5,
    ...
    noiseStrength: 0.72,
    elevationRangeM: 10700,
    elevationNoiseStrength: 0.32,
...
export function createEmptyProject(title = "새로운 세계", options: NewProjectOptions = {}): WorldProject {
  const now = new Date().toISOString();
  const rpgEnabled = Boolean(options.rpgEnabled);
  const magicEnabled = rpgEnabled && Boolean(options.magicEnabled);
  return {
    ...
    worldSettings: {
      orbitalPeriodDays: Math.max(1, Number(options.orbitalPeriodDays ?? 365)),
      simulationMode: "realistic",
      environmentEngine: "builtin",
      latitudeDeg: Math.max(-89, Math.min(89, Number(options.latitudeDeg ?? 38))),
      axialTiltDeg: Math.max(0, Math.min(90, Number(options.axialTiltDeg ?? 23.44))),
      dayLengthHours: Math.max(1, Number(options.dayLengthHours ?? 24)),
      gravityMs2: Math.max(0.1, Number(options.gravityMs2 ?? 9.80665)),
      magicEnabled,
    },
    rpgSettings: createDefaultRpgSettings(rpgEnabled, magicEnabled),
    simulationSummaries: {},
...
export function createDemoProject(): WorldProject {
  const project = createEmptyProject("에테리아 세계", { rpgEnabled: true, magicEnabled: true });
  ...
  const makeEvent = (
    ...
    category,
    description,
    locationMode: location ? "coordinate" : "custom",
    location,
    locationText: "",
    cause: "",
    result: "",
  ...
  const participant = (
    ...
    organizationName,
    representativeLeaderName: keyFigures,
    otherLeaderArticleIds: [],
    otherLeaderNames: [],
    participationScale: scale,
    cause,
    result,
    impact,
  });
  ...
  const person = (title: string, summary: string): WikiArticle => ({
    ...
    rpgData: {
      ...createEmptyRpgArticleData(["character"]),
      values: {
        health: 20,
        strength: 10,
        agility: 12,
  ...
  project.wikiArticles.push(
    ...
    },
    {
      ...
      lastModifiedDate: now,
      religionProfile: {
        traditionLineage: "별길 신앙 계통",
        founder: "초대 길잡이",
        foundingPeriod: "240년경",
        holyCity: "아우렐리아",
        distributionRegions: ["왕국의 주요 항구"],
        adherentPopulation: "약 18만 명",
        religiousInstitutions: [starOrder.name],
        scriptures: ["별길 항해록"],
        majorDenominations: [],
        doctrineSections: [],
      },
  ...
  const demoFamily = project.wikiArticles.find((article) => article.title === "셀레네 가문");
  if (demoFamily) {
    const traitId = createId("rpg-trait");
    demoFamily.rpgData = { ...createEmptyRpgArticleData(["bloodline"]), traitIds: [traitId] };
    project.rpgSettings.traits.push({
      id: traitId,
      name: "은빛 혈통",
      description: "가문의 직계 혈족은 마력 회복이 빠르다.",
      tags: ["혈통", "마력"],
      familyArticleId: demoFamily.id,
      scope: "lineal",
      anchorArticleId: selene.id,
      selectedArticleIds: [],
      includeSpouses: false,
      includeAdopted: false,
      modifiers: [{
        id: createId("rpg-modifier"),
        statId: "mana_regeneration",
        operator: "percent_add",
        value: 50,
        sourceKind: "family",
        sourceArticleId: demoFamily.id,
  ...
  const starCompass = project.wikiArticles.find((article) => article.title === "별나침반");
  if (starCompass) starCompass.rpgData = {
    ...createEmptyRpgArticleData(["general-item"]),
    values: { durability: 72, mana: 40, perception: 8 },
    modifiers: [{ id: createId("rpg-modifier"), statId: "perception", operator: "flat_add", value: 8, sourceKind: "equipment", sourceArticleId: starCompass.id, priority: 100, activationMode: "carried", active: true }],
  };
  ...
  if (familyArticle?.familyProfile) {
    const familyPersonIds = new Set(familyArticle.familyProfile.members.flatMap((member) => member.articleId ? [member.articleId] : []));
    for (const personArticle of project.wikiArticles) {
      if (personArticle.personProfile && familyPersonIds.has(personArticle.id)) {
        personArticle.personProfile.familyArticleId = familyArticle.id;
        personArticle.personProfile.fatherArticleId = undefined;
        personArticle.personProfile.motherArticleId = undefined;
        personArticle.personProfile.spouseArticleIds = [];
        personArticle.personProfile.childArticleIds = [];
      }
    }
  }
  ...
  const demoArticle = (
    ...
    if (category === "culture" || category === "culture_sphere")
      base.cultureProfile = {
        distributionRegions: [], relatedLanguageArticleIds: [],
        relatedReligionArticleIds: [], characteristicSections: [],
      };
    if (["technology", "technology_engineering", "technology_mathematics", "technology_science", "technology_chemistry", "technology_medicine", "technology_physics"].includes(category))
      base.technologyProfile = { creator: "", creationPeriod: "", characteristicSections: [] };
