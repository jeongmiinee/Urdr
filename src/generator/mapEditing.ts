...
export function applyElevationBrushImmediate(data: GeneratedMapData, point: Point, radiusWorld: number, delta: number): GeneratedMapData {
  ...
  return { ...data, elevationMap: elevation, generatedAt: new Date().toISOString() };
}
...
function traceRasterUnion(existingPolygon: Point[], brushPolygon: Point[]): Point[] | null {
  ...
  for (let y = 0; y < gridHeight; y += 1) {
    for (let x = 0; x < gridWidth; x += 1) {
      ...
      if ((existingPolygon.length >= 3 && pointInPolygon(point, existingPolygon)) || pointInPolygon(point, brushPolygon)) {
...
export function buildTerritoryPolygon(existingPolygon: Point[], brushPoints: Point[]): Point[] {
  ...
  const traced = traceRasterUnion(existingPolygon, brushPoints);
