...
function terrainFor(
  ...
  const localMap = settings.mapScaleKm <= 100;
  const mountainLocal = localMap && settings.localRegionType === "mountain";
  const slopeStart = mountainLocal ? 0.055 : localMap ? 0.095 : 0.045;
  const slopeRange = mountainLocal ? 0.24 : localMap ? 0.42 : 0.22;
  const slopeFactor = clamp((slope - slopeStart) / slopeRange);
  ...
  if (localMap && !mountainLocal) {
    scores.rock = (scores.rock ?? 0) - 0.9 - clamp((0.36 - normalized) / 0.36) * 0.85;
    scores.mountain = (scores.mountain ?? 0) - 0.65;
    scores.plain = (scores.plain ?? 0) + 0.28;
    scores.grassland = (scores.grassland ?? 0) + 0.18;
  }
...
export function calculateEnvironment(
  ...
  const broadScale = Math.max(18, minimumDimension * 0.18 * patchScale);
  const regionalScale = Math.max(8, broadScale * 0.43);
  const forestPotential = new Float32Array(size);
  const potentialHistogram = new Uint32Array(160);
  let eligibleForestCells = 0;
  let climateSuitabilityTotal = 0;

  // A normalized target keeps coverage stable while the score moves forest cores inland.
  for (let index = 0; index < size; index += 1) {
    const terrain = terrainMap[index];
    if (!["plain", "grassland", "forest"].includes(terrain) || elevation[index] <= seaLevel) continue;
    const x = index % width;
    const y = Math.floor(index / width);
    const temperatureSuitability = clamp(1 - Math.abs(smoothedTemperature[index] - 14) / 21);
    const precipitationSuitability = clamp((smoothedPrecipitation[index] - 260) / 1_350);
    const moistureSuitability = clamp((smoothedMoisture[index] - 0.22) / 0.68);
    const runoffSuitability = clamp(smoothedRunoff[index] / 950);
    const elevationSuitability = clamp(1 - Math.max(0, elevation[index] - seaLevel) / Math.max(900, settings.maxElevation * 0.72));
    const slopeSuitability = clamp(1 - Math.hypot(gradientX[index], gradientY[index]) / 0.52);
    const coastDistance = Number.isFinite(distances[index]) ? distances[index] : minimumDimension;
    const continentalDistance = clamp(coastDistance / Math.max(4, minimumDimension * 0.28));
    const broadCluster = fractalPerlinNoise(x / broadScale, y / broadScale, settings.seed + 44_117, 5, 0.59, 2.01);
    const regionalCluster = fractalPerlinNoise(x / regionalScale, y / regionalScale, settings.seed + 44_191, 4, 0.54, 2.08);
    const cluster = clamp(broadCluster * 0.68 + regionalCluster * 0.32);
    const climateSuitability = clamp(
      temperatureSuitability * 0.19
        + precipitationSuitability * 0.28
        + moistureSuitability * 0.25
        + runoffSuitability * 0.08
        + elevationSuitability * 0.1
        + slopeSuitability * 0.1,
    );
    const inlandCore = clamp((continentalDistance - 0.08) / 0.72);
    const potential = clamp(cluster * 0.5 + climateSuitability * 0.34 + inlandCore * 0.22 - (1 - continentalDistance) * 0.1);
    forestPotential[index] = potential;
    potentialHistogram[Math.min(potentialHistogram.length - 1, Math.floor(potential * potentialHistogram.length))] += 1;
    climateSuitabilityTotal += climateSuitability;
    eligibleForestCells += 1;
  }

  const meanForestClimate = climateSuitabilityTotal / Math.max(1, eligibleForestCells);
  const areaAdjustment = clamp(Math.log10(physicalAreaKm2 + 10) / 8, 0.12, 0.78);
  const targetForestCoverage = clamp(0.12 + meanForestClimate * 0.24 + areaAdjustment * 0.035, 0.12, 0.38);
  const targetForestCells = Math.round(eligibleForestCells * targetForestCoverage);
  let accumulated = 0;
  let thresholdBucket = potentialHistogram.length - 1;
  for (; thresholdBucket > 0; thresholdBucket -= 1) {
    accumulated += potentialHistogram[thresholdBucket];
    if (accumulated >= targetForestCells) break;
  }
  const forestThreshold = thresholdBucket / potentialHistogram.length;
  terrainMap = terrainMap.map((terrain, index) => {
    if (!["plain", "grassland", "forest"].includes(terrain) || elevation[index] <= seaLevel) return terrain;
    const x = index % width;
    const y = Math.floor(index / width);
    const edgeVariation = (deterministicCellRandom(x, y, settings.seed + 91_331) - 0.5) * 0.055;
    if (forestPotential[index] + edgeVariation >= forestThreshold) return "forest";
    return terrain === "forest" ? (smoothedMoisture[index] > 0.46 ? "grassland" : "plain") : terrain;
  });
  ...
  // 희박한 숲 조각은 걷어내고 군집 내부의 공백을 메워 최소 밀집도를 높인다.
  for (let pass = 0; pass < 2; pass += 1) {
    const consolidated = [...terrainMap];
    for (let y = 1; y < height - 1; y += 1) {
      for (let x = 1; x < width - 1; x += 1) {
        const index = y * width + x;
        if (!["plain", "grassland", "forest"].includes(terrainMap[index])) continue;
        let forestNeighbors = 0;
        for (let oy = -1; oy <= 1; oy += 1) for (let ox = -1; ox <= 1; ox += 1) {
          if (ox === 0 && oy === 0) continue;
          if (terrainMap[(y + oy) * width + x + ox] === "forest") forestNeighbors += 1;
        }
        const suitable = smoothedTemperature[index] > -4 && smoothedTemperature[index] < 31
          && smoothedPrecipitation[index] > 360 && smoothedMoisture[index] > 0.31;
        if (terrainMap[index] === "forest" && forestNeighbors < 2) {
          consolidated[index] = smoothedMoisture[index] > 0.48 ? "grassland" : "plain";
        } else if (terrainMap[index] !== "forest" && suitable && forestNeighbors >= 4) {
          consolidated[index] = "forest";
        }
      }
    }
    terrainMap = consolidated;
  }

  const minimumForestArea = Math.max(18, Math.round(width * height * 0.0003));
  const forestComponents = landComponents(terrainMap.map((terrain) => terrain === "forest"), width, height);
  for (const component of forestComponents) if (component.size < minimumForestArea) {
    for (const index of component.cells) {
      terrainMap[index] = smoothedMoisture[index] > 0.48 ? "grassland" : "plain";
    }
  }
  ...
  const localOrRegional = settings.mapScope === "local" || settings.mapScope === "regional";
  if (localOrRegional) {
    const landCells = terrainMap.reduce((count, _terrain, index) => count + (elevation[index] > seaLevel ? 1 : 0), 0);
    const mountainRegion = settings.localRegionType === "mountain";
    const maximumRockCells = Math.round(landCells * (mountainRegion ? 0.56 : 0.24));
    let rockCells = terrainMap.reduce((count, terrain, index) =>
      count + (elevation[index] > seaLevel && (terrain === "rock" || terrain === "bedrock") ? 1 : 0), 0);
    if (rockCells > maximumRockCells) {
      for (let index = 0; index < terrainMap.length && rockCells > maximumRockCells; index += 1) {
        if (elevation[index] <= seaLevel || (terrainMap[index] !== "rock" && terrainMap[index] !== "bedrock")) continue;
        const slope = Math.hypot(gradientX[index], gradientY[index]);
        const relativeElevation = elevation[index] - seaLevel;
        if (slope > (mountainRegion ? 0.5 : 0.34) || relativeElevation > settings.maxElevation * (mountainRegion ? 0.72 : 0.42)) continue;
        terrainMap[index] = smoothedMoisture[index] > 0.42 ? "grassland" : "plain";
        rockCells -= 1;
      }
    }
  }
