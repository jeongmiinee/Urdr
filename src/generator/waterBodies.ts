...
export function classifyWaterBodies(
  ...
  // Only frame-connected water is authoritative ocean. Enclosed depressions
  // remain land candidates until the hydrology solver validates a real lake.
  for (let index = 0; index < elevation.length; index += 1)
    result[index] = salt[index] ? "saltwater" : "land";
  ...
  if (settings.mapShape === "inland_sea") {
    ...
    for (let start = 0; start < elevation.length; start += 1) {
      if (elevation[start] > seaLevel || salt[start] || visited[start]) continue;
      const cells: number[] = [];
      head = 0;
      tail = 0;
      visited[start] = 1;
      ...
      while (head < tail) {
        ...
        for (const [dx, dy] of dirs) {
          ...
          if (elevation[next] <= seaLevel && !salt[next] && !visited[next]) {
            visited[next] = 1;
            queue[tail++] = next;
          }
        }
