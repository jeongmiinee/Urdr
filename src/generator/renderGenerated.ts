...
export function drawGeneratedPreviewOverlays(
  ...
  if (data.rivers?.length) {
    ...
    for (const river of [...data.rivers].sort((a, b) => a.width - b.width)) {
      context.beginPath();
      context.moveTo(river.start.x * scaleX, river.start.y * scaleY);
      context.lineTo(river.end.x * scaleX, river.end.y * scaleY);
      context.lineWidth = Math.max(0.55, river.width * canvas.width / 960);
      context.stroke();
    }
  }
...
export function drawGeneratedPreview(
  canvas: HTMLCanvasElement,
  data: GeneratedMapData,
  showContours = true,
  showCoastline = false,
  ...
  const context = canvas.getContext("2d");
  if (!context) throw new Error("2D Canvas 컨텍스트를 만들 수 없습니다.");
  const safeWidth = Math.max(320, Math.min(1024, Math.round(targetWidth || 960)));
  const safeHeight = Math.max(160, Math.min(1024, Math.round(targetHeight || safeWidth * data.gridHeight / data.gridWidth)));
  const base = createGeneratedMapCanvas(data, 1024, safeWidth, safeHeight);
  canvas.width = safeWidth;
  canvas.height = Math.max(1, Math.round(safeWidth * data.gridHeight / data.gridWidth));
  context.imageSmoothingEnabled = true;
