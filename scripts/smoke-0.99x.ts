...
assert(PROGRAM_VERSION === "0.99x", "프로그램 버전이 0.99x가 아님");
...
assert(countryResult.generated.terrainMap.includes("farmland"), "도시 배치 후 농경지가 생성되지 않음");
...
assert(demo.version === "0.99x", "데모 버전이 0.99x가 아님");
...
assert(demoGenerated?.terrainMap?.includes("farmland"), "데모 도시 주변 농경지가 생성되지 않음");
