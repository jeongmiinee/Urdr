...
import { createDemoProject, createId, defaultGeneratorSettings, type WikiArticle, type WorldEvent } from "../src/model/world";
import { activeMap } from "../src/model/worldSelectors";
...
const settings = {
  ...
  coastlineDetail: 1.0,
  noiseStrength: 1.0,
  hybridPerlinWarp: 0.86,
  hybridPolygonRefinement: 1.9,
  coastSmoothness: 0.42,
  contourInterval: 150,
  riverNodeCount: 36,
};

// 이전 데모 영토를 재사용하지 않는다. 새 지형과 도시 배치가 끝난 뒤
// 도시 발원 상승·완전 침식·재팽창으로 국가 영토를 전부 새로 만든다.
map.territories = [];
...
const regeneratedCountryIds = new Set(countries.map((country) => country.id));
const generatedCountryTerritories = demoMap.territories.filter((territory) => territory.id.startsWith("generated-territory-"));
if (generatedCountryTerritories.length === 0) throw new Error("새 데모 국가 영토가 생성되지 않았습니다.");
if (demoMap.territories.some((territory) => !territory.id.startsWith("generated-territory-"))) {
  throw new Error("이전 데모 영토 또는 수동 영토가 재사용되었습니다.");
}
const territoryOwnerIds = new Set(generatedCountryTerritories.flatMap((territory) => territory.states.map((state) => state.value.ownerFactionId).filter(Boolean)));
if ([...regeneratedCountryIds].some((countryId) => !territoryOwnerIds.has(countryId))) {
  throw new Error("영토가 새로 생성되지 않은 데모 국가가 있습니다.");
}
if (countries.some((country) => !country.flagAssetId || !country.coatOfArmsAssetId)) {
  throw new Error("깃발 또는 문장이 없는 데모 국가가 있습니다.");
}
if (!result.generated.surfaceRegions?.length) throw new Error("데모 지형 벡터 영역이 생성되지 않았습니다.");
if (!result.generated.agricultureMap?.some((value) => value >= 0.42)) throw new Error("도시·수계 기반 농경지가 생성되지 않았습니다.");

const serialized = JSON.stringify(synced);
await writeFile(new URL("../public/demoProjectData.json.gz", import.meta.url), gzipSync(serialized, { level: 9 }));
console.log(JSON.stringify({
  ...
  compressedBytes: gzipSync(serialized, { level: 9 }).length,
}, null, 2));
